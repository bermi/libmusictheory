...
| `playability.phrase.{KeyboardPhraseEvent,FretPhraseEvent,PhraseIssue,PhraseSummary,PhraseAuditResult,SummaryAccumulator,summarizeIssues,auditKeyboardPhrase,auditFretPhrase}` | realized events, issue rows, event count, hand or tuning context, output buffers | fixed-size event rows, summarized phrase facts, audit results with logical/written issue counts | `playability.phrase.auditKeyboardPhrase(events[0..], profile, issues[0..])` | Run a fixed-realization phrase audit that emits event-local issues, transition-local issues, repeated warning clusters, and recovery-deficit runs without rewriting the music. |
...
| `lmt_summarize_playability_phrase_issues`, `lmt_audit_keyboard_phrase_n`, `lmt_audit_fret_phrase_n` | event count, phrase events or issue rows, hand/tuning context, optional issue buffers, output summary | success flag or logical issue count | `lmt_audit_keyboard_phrase_n(events, event_count, &profile, issues, cap, &summary)` | Audit fixed-realization phrases through the library so hosts can explain warning clusters, hand continuity reset, and recovery-deficit runs without reconstructing phrase logic in UI code. |
