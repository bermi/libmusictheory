...
| `playability.phrase.{KeyboardPhraseEvent,FretPhraseEvent,KeyboardPhraseBranch,FretPhraseBranch,KeyboardPhraseStepCandidates,FretPhraseStepCandidates,KeyboardPhraseCandidateWindow,FretPhraseCandidateWindow,KeyboardCommittedPhraseMemory,FretCommittedPhraseMemory,PhraseIssue,PhraseSummary,PhraseBranchSummary,PhraseAuditResult,SummaryAccumulator,summarizeIssues,summarizeBranchIssues,summarizeKeyboardBranch,summarizeFretBranch,auditKeyboardPhrase,auditFretPhrase,auditCommittedKeyboardPhrase,auditCommittedFretPhrase}` | realized events, fixed-horizon branch containers, caller-owned committed memory, issue rows, event count, hand or tuning context, output buffers | fixed-size event rows, branch windows, committed phrase windows, summarized phrase and branch facts, audit results with logical/written issue counts | `playability.phrase.summarizeKeyboardBranch(&branch, profile)` | Audit fixed realizations and explicit short branches so later generation slices can explain where a future becomes blocked without hiding ranking policy inside the library. |
| `playability.repair.{RepairClass,RepairPolicy,RankedKeyboardPhraseRepair,RankedFretPhraseRepair,RankedKeyboardPhraseBranchRepair,RankedFretPhraseBranchRepair,MAX_PHRASE_REPAIRS,defaultForClass,rankKeyboardPhraseRepairs,rankFretPhraseRepairs,rankKeyboardPhraseBranchRepairs,rankFretPhraseBranchRepairs}` | committed phrase memory, explicit branch structs, repair policy, hand/tuning context, output buffers | ranked phrase and branch repairs with before/after summaries, what changed, what was preserved, bottleneck-relief metadata, and crossed musical-change boundary flags | `playability.repair.rankKeyboardPhraseBranchRepairs(&memory, &branch, profile, policy, out[0..])` | Generate explainable branch repairs without mixing preservation classes in one list, so hosts can say whether a fix stayed `realization_only`, changed register, or reduced texture across a short continuation. |
| `playability.profile.fromInt`, `playability.profile.applyPreset`, `playability.profile.summarizeFretRealization`, `playability.profile.summarizeFretTransition`, `playability.profile.summarizeKeyboardRealization`, `playability.profile.summarizeKeyboardTransition`, `playability.profile.suggestEasierFretRealization`, `playability.profile.suggestEasierKeyboardFingering`, `playability.profile.suggestSaferKeyboardNextStep`, `playability.profile.suggestSaferKeyboardNextStepFromCommittedPhrase` | preset IDs, hand profiles, assessments, history windows, committed phrase memory | adjusted profiles, difficulty summaries, easier realizations, safer next-step rows | `playability.profile.suggestSaferKeyboardNextStepFromCommittedPhrase(&memory, &history, .tonal_chorale, profile, .balanced)` | Apply explainable ergonomic presets and turn raw assessments into practice-facing summaries and safer suggestions. |
| `playability.ranking.fromInt`, `playability.ranking.rankKeyboardNextSteps`, `playability.ranking.rankKeyboardNextStepsFromCommittedPhrase`, `playability.ranking.filterNextStepsByPlayability`, `playability.ranking.rankKeyboardContextSuggestions`, `playability.ranking.rankKeyboardContextSuggestionsFromCommittedPhrase`, `playability.ranking.{PhraseBranchClassification,PhraseBranchVisibility,PhraseBranchBiasReason,PhraseBranchBiasSummary,RankedKeyboardPhraseBranch,RankedFretPhraseBranch,classifyBranchSummary,summarizeKeyboardBranchBiasFromCommittedPhrase,summarizeFretBranchBiasFromCommittedPhrase,rankKeyboardPhraseBranchesFromCommittedPhrase,rankFretPhraseBranchesFromCommittedPhrase,filterBlockedKeyboardPhraseBranchesFromCommittedPhrase,filterBlockedFretPhraseBranchesFromCommittedPhrase}` | voiced history, theory profile, hand role, hand profile, policy, committed phrase memory, branch sets, output buffers | policy IDs, ranked playability rows, accepted next steps, ranked context rows, branch classifications, visibility modes, bias summaries, hard-filtered branch subsets | `playability.ranking.rankKeyboardPhraseBranchesFromCommittedPhrase(&memory, branches, profile, .balanced, .diagnostics_keep_blocked, out[0..])` | Keep hard blockers and soft strain bias separate: hosts can keep blocked branches visible for diagnostics, or say a branch was blocked only because you asked for hard blockers to be removed, while still explaining when a candidate compounds the same shift strain already present in committed memory. |
...
| `lmt_sizeof_hand_profile`, `lmt_sizeof_temporal_load_state`, `lmt_sizeof_keyboard_phrase_event`, `lmt_sizeof_fret_phrase_event`, `lmt_sizeof_keyboard_phrase_branch`, `lmt_sizeof_fret_phrase_branch`, `lmt_sizeof_keyboard_phrase_step_candidates`, `lmt_sizeof_fret_phrase_step_candidates`, `lmt_sizeof_keyboard_phrase_candidate_window`, `lmt_sizeof_fret_phrase_candidate_window`, `lmt_sizeof_keyboard_committed_phrase_memory`, `lmt_sizeof_fret_committed_phrase_memory`, `lmt_sizeof_playability_phrase_issue`, `lmt_sizeof_playability_phrase_summary`, `lmt_sizeof_playability_phrase_branch_summary`, `lmt_sizeof_fret_candidate_location`, `lmt_sizeof_fret_play_state`, `lmt_sizeof_fret_realization_assessment`, `lmt_sizeof_fret_transition_assessment`, `lmt_sizeof_ranked_fret_realization`, `lmt_sizeof_keybed_key_coord`, `lmt_sizeof_keyboard_play_state`, `lmt_sizeof_keyboard_realization_assessment`, `lmt_sizeof_keyboard_transition_assessment`, `lmt_sizeof_ranked_keyboard_fingering`, `lmt_sizeof_ranked_keyboard_context_suggestion`, `lmt_sizeof_ranked_keyboard_next_step`, `lmt_sizeof_playability_difficulty_summary` | none | byte counts | `lmt_sizeof_keyboard_phrase_candidate_window()` | Guard FFI layout compatibility for every playability, phrase-audit, and phrase-branch struct a host may exchange with the library. |
...
| `lmt_keyboard_committed_phrase_reset`, `lmt_keyboard_committed_phrase_push`, `lmt_keyboard_committed_phrase_len`, `lmt_fret_committed_phrase_reset`, `lmt_fret_committed_phrase_push`, `lmt_fret_committed_phrase_len` | committed phrase memory pointers and events | reset side effects or logical event counts | `lmt_keyboard_committed_phrase_push(&memory, &event)` | Maintain caller-owned committed memory for accepted phrase events while keeping preview-only host interactions out of library memory. |
| `lmt_summarize_playability_phrase_issues`, `lmt_summarize_playability_phrase_branch_issues`, `lmt_audit_keyboard_phrase_n`, `lmt_audit_fret_phrase_n`, `lmt_summarize_keyboard_phrase_branch_n`, `lmt_summarize_fret_phrase_branch_n`, `lmt_audit_committed_keyboard_phrase_n`, `lmt_audit_committed_fret_phrase_n` | event or step count, phrase events or issue rows, explicit branch structs, committed phrase memory, hand/tuning context, optional issue buffers, output summary | success flag or logical issue count | `lmt_summarize_keyboard_phrase_branch_n(&branch, &profile, &summary)` | Audit fixed-realization phrases and explicit short branches through the library so hosts can explain where a future becomes blocked, where peak strain lands, and how many future windows remain recovery-deficit windows. |
| `lmt_playability_repair_class_count`, `lmt_playability_repair_class_name`, `lmt_default_playability_repair_policy`, `lmt_rank_keyboard_phrase_repairs_n`, `lmt_rank_fret_phrase_repairs_n`, `lmt_rank_keyboard_phrase_branch_repairs_n`, `lmt_rank_fret_phrase_branch_repairs_n` | repair class IDs, committed phrase memory, explicit branch structs, repair policy, hand/tuning context, output buffers | repair policy structs, ranked phrase and branch repairs, logical counts | `lmt_rank_keyboard_phrase_branch_repairs_n(&memory, &branch, &profile, &policy, out, cap)` | Return ranked phrase and branch repairs with explicit repair policy, crossed musical-change boundary flags, what changed, what was preserved, and which branch bottleneck moved, so hosts can explain why a candidate stayed `realization_only` or moved into `register_adjusted`/`texture_reduced`. |
...
| `lmt_suggest_easier_fret_realization_n`, `lmt_suggest_easier_keyboard_fingering_n`, `lmt_filter_next_steps_by_playability`, `lmt_rank_keyboard_next_steps_by_playability`, `lmt_rank_keyboard_next_steps_by_committed_phrase`, `lmt_suggest_safer_keyboard_next_step_by_playability`, `lmt_suggest_safer_keyboard_next_step_by_committed_phrase`, `lmt_rank_keyboard_context_suggestions_by_playability`, `lmt_rank_keyboard_context_suggestions_by_committed_phrase`, `lmt_playability_phrase_branch_class_count`, `lmt_playability_phrase_branch_class_name`, `lmt_playability_phrase_branch_visibility_count`, `lmt_playability_phrase_branch_visibility_name`, `lmt_playability_phrase_branch_bias_reason_count`, `lmt_playability_phrase_branch_bias_reason_name`, `lmt_rank_keyboard_phrase_branches_by_committed_phrase`, `lmt_rank_fret_phrase_branches_by_committed_phrase`, `lmt_hard_filter_keyboard_phrase_branches_by_committed_phrase`, `lmt_hard_filter_fret_phrase_branches_by_committed_phrase` | current note/fret context, theory profile, hand role, hand profile, policy, committed phrase memory, explicit branch sets, output buffers | ranked rows, filtered next steps, branch diagnostics, or one safer fallback | `lmt_rank_keyboard_phrase_branches_by_committed_phrase(&memory, branches, branch_count, &profile, LMT_PLAYABILITY_POLICY_MINIMAX_BOTTLENECK, LMT_PLAYABILITY_PHRASE_BRANCH_KEEP_BLOCKED, out, cap)` | Turn theory-valid output into explicitly playable alternatives and safer continuations for practice tools and LLM assistants, with hard blockers and soft strain bias separate so a host can keep blocked branches visible for diagnostics or remove them intentionally. |
...
#### Recipe 6: Keep Diagnostics Visible Before Hard Filtering

```c
lmt_ranked_keyboard_phrase_branch ranked[8] = {0};
lmt_keyboard_phrase_branch accepted_only[8] = {0};

/* 1. Diagnostic pass: keep blocked branches visible. */
uint32_t diagnostic_total = lmt_rank_keyboard_phrase_branches_by_committed_phrase(
    &memory,
    branches,
    branch_count,
    &tuned,
    LMT_PLAYABILITY_POLICY_MINIMAX_BOTTLENECK,
    LMT_PLAYABILITY_PHRASE_BRANCH_KEEP_BLOCKED,
    ranked,
    8);

/* 2. Separate acceptance pass: remove only hard blockers. */
uint32_t accepted_total = lmt_hard_filter_keyboard_phrase_branches_by_committed_phrase(
    &memory,
    branches,
    branch_count,
    &tuned,
...
`"I kept blocked branches visible first so I could explain the diagnostics. This one compounds the same shift strain already present in committed memory. That branch was blocked only because you asked for hard blockers to be removed."`

#### Recipe 7: Request One Preservation Class Of Branch Repair At A Time

```c
lmt_ranked_keyboard_phrase_branch_repair branch_repairs[8] = {0};

lmt_default_playability_repair_policy(
    LMT_PLAYABILITY_REPAIR_REALIZATION_ONLY,
    &policy);

uint32_t logical_branch_repairs = lmt_rank_keyboard_phrase_branch_repairs_n(
    &memory,
    &branch,
    &tuned,
    &policy,
...
`"I asked for realization-only branch repairs first, so every candidate keeps the sounding notes and only changes the physical realization across the branch. If that does not relieve the bottleneck, I can make a second call that explicitly widens the boundary to register-adjusted or texture-reduced repairs."`

The branch-repair rows keep the explanation surface explicit:

- `events_touched` tells you how much of the short continuation changed
- `notes_changed` stays `0` when only the physical realization changed
- `first_relieved_bottleneck_step_index` tells you where the branch stopped failing first
- `new_dominant_domain` and `new_dominant_family_index` tell you whether a new warning or reason family became dominant after the rewrite

#### Recipe 8: LLM Explanation Pattern
