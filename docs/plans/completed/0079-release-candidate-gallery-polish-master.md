...
- Shipped behavior:
  ...
  - added reproducible gallery screenshot capture and Playwright verification for large, presentation-grade previews
  - corrected the public staff renderer so gallery captures depend on real notation features including simultaneous chord clustering, shared stems, and proper clef openings
  - cut and documented the first standalone release candidate as `0.1.0-rc.1` with reviewer guidance and release smoke gates
