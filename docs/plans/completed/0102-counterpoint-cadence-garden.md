...
- `2d27252d791ab47d64b6157a4a7ac62928a9e1b7` — `2026-04-04`
  - shipped the live `Cadence Garden` card in the interactive gallery, grouping recursively ranked continuation paths by reachable cadence outcome from the focused or pinned next move
  - added cadence-group summaries with representative terminal clock previews, warning counts, branch counts, and mini instrument support
  - tightened gallery and `verify.sh` guardrails so hover, pin, clear-pin, context changes, profile changes, and preview-mode changes must keep cadence groups populated and synchronized
