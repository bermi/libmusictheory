...
- Tighten `./verify.sh` so `scale` is required in the sampled native-RGBA proof lane.
- Add deterministic native candidate/reference bitmap support for `scale` and close `0057`.
