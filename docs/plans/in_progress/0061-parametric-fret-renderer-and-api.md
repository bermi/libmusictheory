...
The existing `lmt_svg_fret` surface remains as a six-string compatibility wrapper. New `*_n` APIs will expose:
...
- new C/WASM ABI:
  ...
  - `lmt_svg_fret_n`
- wasm docs/demo updates so the interactive surface stops implying six-string-only support
