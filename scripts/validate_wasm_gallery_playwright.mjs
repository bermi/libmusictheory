  ...
  try {
    ...
    try {
      ...
      if (
        ...
        || (midiActiveRepairFutureFeatures?.rowCount || 0) < 2
        || (midiActiveRepairFutureFeatures?.concreteFutureCount || 0) < 1
        || (midiActiveRepairFutureFeatures?.improvedFutureCount || 0) < 1
      ) {
      ...
      if (
        ...
        || (midiActiveRepairFutureFeatures?.concreteFutureCount || 0) < 1
        || (midiActiveRepairFutureFeatures?.improvedFutureCount || 0) < 1
        || (midiActiveRepairFutureFeatures?.futureClockCount || 0) < 1
        || (midiActiveRepairFutureFeatures?.futureLabels?.length || 0) < 2
        || (midiActiveRepairFutureFeatures?.focusedSignature || "") !== (midiActive.summary?.focusedSuggestionSignature || "")
      ) {
        throw new Error(`live midi scene did not render repair futures correctly: ${JSON.stringify(midiActiveRepairFutureFeatures)}`);
      }
      if (
      ...
      const hoveredCandidate = await page.waitForFunction(({ targetIndex, targetSignature }) => {
        ...
        return midi?.hoveredCandidateIndex === targetIndex
          ...
          && (midi?.midiRepairFutureFeatures?.focusedSignature || "") === targetSignature
          && (midi?.midiRepairFutureFeatures?.rowCount || 0) >= 2
          && (midi?.midiRepairFutureFeatures?.concreteFutureCount || 0) >= 1
          && (midi?.midiRepairFutureFeatures?.improvedFutureCount || 0) >= 1
      ...
      const hoverReset = await page.waitForFunction(() => {
        ...
        return midi?.pinnedCandidateIndex === -1
          ...
          && (midi?.midiRepairLabFeatures?.rowCount || 0) >= 2
          && (midi?.midiRepairLabFeatures?.improvedRepairCount || 0) >= 1
          && (midi?.midiRepairFutureFeatures?.focusedSignature || "") === (midi?.focusedSuggestionSignature || "")
          && (midi?.midiRepairFutureFeatures?.rowCount || 0) >= 2
          && (midi?.midiRepairFutureFeatures?.concreteFutureCount || 0) >= 1
          && (midi?.midiRepairFutureFeatures?.improvedFutureCount || 0) >= 1;
      }, { timeout: 30000 }).then((handle) => handle.jsonValue());
