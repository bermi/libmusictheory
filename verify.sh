...
if [ -f "$ROOT_DIR/docs/plans/in_progress/0087-public-api-polish-and-review-fixes.md" ] || [ -f "$ROOT_DIR/docs/plans/completed/0087-public-api-polish-and-review-fixes.md" ]; then
    ...
    check_cmd "cd '$ROOT_DIR' && rg -n 'lmt_svg_clock_optc|lmt_svg_fret\\b|lmt_svg_fret_n|lmt_svg_chord_staff' examples/wasm-demo/index.html examples/wasm-demo/app.js examples/wasm-demo/qa-atlas.js >/dev/null" "0087 qa atlas guardrail (all public docs image methods are rendered and atlas-covered)"
    check_cmd "cd '$ROOT_DIR' && rg -n 'lmt_bitmap_clock_optc_rgba|lmt_bitmap_fret_rgba|lmt_bitmap_fret_n_rgba|lmt_bitmap_chord_staff_rgba' include/libmusictheory.h build.zig scripts/check_wasm_exports.mjs examples/wasm-demo/qa-atlas.js >/dev/null" "0087 qa atlas guardrail (public bitmap image APIs are declared, exported, and atlas-consumed)"
    check_cmd "cd '$ROOT_DIR' && rg -n 'ImageData|putImageData|createObjectURL|toBlob|target=\"_blank\"|BITMAP_REVIEW_WIDTH' examples/wasm-demo/qa-atlas.js examples/wasm-demo/qa-atlas.html >/dev/null && ! rg -n 'outerHTML|svgMarkup|drawImage\\(' examples/wasm-demo/qa-atlas.js" "0087 qa atlas guardrail (atlas renders direct library RGBA bitmaps with open-in-browser links)"
