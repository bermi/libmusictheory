...
if [ -f "$ROOT_DIR/docs/plans/in_progress/0124-instrument-topology-and-biomechanical-state.md" ] || [ -f "$ROOT_DIR/docs/plans/completed/0124-instrument-topology-and-biomechanical-state.md" ]; then
    check_cmd "cd '$ROOT_DIR' && test -f src/playability.zig && test -f src/playability/types.zig && test -f src/playability/fret_topology.zig && test -f src/playability/keyboard_topology.zig && test -f src/tests/playability_types_test.zig && test -f src/tests/playability_topology_test.zig && rg -n 'pub const playability|tests/playability_types_test\\.zig|tests/playability_topology_test\\.zig' src/root.zig >/dev/null" "0124 playability foundation guardrail (module family and focused tests are wired into root)"
    check_cmd "cd '$ROOT_DIR' && test -f docs/research/algorithms/playability.md && rg -n 'Hori and Sagayama|Topological Considerations|Variable Neighborhood Search|Checklist Models|playability' docs/research/algorithms/playability.md >/dev/null" "0124 playability research guardrail (foundational rationale is documented in research notes)"
    check_cmd "cd '$ROOT_DIR' && rg -n 'lmt_playability_reason|lmt_playability_warning|lmt_hand_profile|lmt_temporal_load_state|lmt_fret_candidate_location|lmt_fret_play_state|lmt_keybed_key_coord|lmt_keyboard_play_state|lmt_default_fret_hand_profile|lmt_default_keyboard_hand_profile|lmt_describe_fret_play_state|lmt_windowed_fret_positions_n|lmt_keyboard_key_coord|lmt_describe_keyboard_play_state' include/libmusictheory.h src/c_api.zig build.zig scripts/check_wasm_exports.mjs src/tests/c_api_test.zig >/dev/null" "0124 playability ABI guardrail (experimental structs, exports, and C tests are wired)"
...
if [ -f "$ROOT_DIR/docs/plans/in_progress/0129-personalized-profiles-and-practice-feedback.md" ] || [ -f "$ROOT_DIR/docs/plans/completed/0129-personalized-profiles-and-practice-feedback.md" ]; then
    check_cmd "cd '$ROOT_DIR' && test -f src/playability/profile.zig && test -f src/tests/playability_profile_test.zig && rg -n 'pub const profile|tests/playability_profile_test\\.zig' src/playability.zig src/root.zig >/dev/null" "0129 playability profile foundation guardrail (profile module and focused tests are wired)"
...
if [ -f "$ROOT_DIR/docs/plans/in_progress/0131-playability-gallery-screenshots-and-captions.md" ] || [ -f "$ROOT_DIR/docs/plans/completed/0131-playability-gallery-screenshots-and-captions.md" ]; then
    check_cmd "cd '$ROOT_DIR' && rg -n 'scene-midi-playability-guide\\.png|scene-midi-playability-piano\\.png|scene-midi-playability-fret\\.png|Playability Gallery States' README.md docs/api.md docs/release/artifacts.md docs/release/gallery-capture.md docs/release/reviewer-guide.md >/dev/null" "0131 playability screenshot docs guardrail (README and release docs reference the dedicated playability captures)"
    check_cmd "cd '$ROOT_DIR' && test -f docs/release/images/scene-midi-playability-guide.png && test -f docs/release/images/scene-midi-playability-piano.png && test -f docs/release/images/scene-midi-playability-fret.png" "0131 playability screenshot asset guardrail (curated deterministic doc images are checked in)"
...
if [ -f "$ROOT_DIR/docs/plans/in_progress/0133-phrase-event-model-and-audit-summaries.md" ] || [ -f "$ROOT_DIR/docs/plans/completed/0133-phrase-event-model-and-audit-summaries.md" ]; then
    check_cmd "cd '$ROOT_DIR' && test -f src/playability/phrase.zig && test -f src/tests/playability_phrase_test.zig && rg -n 'pub const phrase|tests/playability_phrase_test\\.zig' src/playability.zig src/root.zig >/dev/null" "0133 phrase foundation guardrail (module and focused tests are wired)"
    check_cmd "cd '$ROOT_DIR' && rg -n 'KeyboardPhraseEvent|FretPhraseEvent|PhraseIssue|PhraseSummary|IssueScope|IssueSeverity|FamilyDomain|StrainBucket|summarizeIssues' src/playability/phrase.zig src/tests/playability_phrase_test.zig >/dev/null" "0133 phrase algorithm guardrail (event model, issue vocabulary, and summary reducer are implemented)"
    check_cmd "cd '$ROOT_DIR' && rg -n 'lmt_playability_phrase_issue_scope|lmt_playability_phrase_issue_severity|lmt_playability_phrase_family_domain|lmt_playability_phrase_strain_bucket|lmt_keyboard_phrase_event|lmt_fret_phrase_event|lmt_playability_phrase_issue|lmt_playability_phrase_summary|lmt_sizeof_keyboard_phrase_event|lmt_sizeof_fret_phrase_event|lmt_sizeof_playability_phrase_issue|lmt_sizeof_playability_phrase_summary|lmt_summarize_playability_phrase_issues' include/libmusictheory.h src/c_api.zig build.zig scripts/check_wasm_exports.mjs src/tests/c_api_test.zig >/dev/null" "0133 phrase ABI guardrail (structs, reflection, sizeof helpers, and summary wrapper are wired)"
...
if [ -f "$ROOT_DIR/docs/plans/in_progress/0135-committed-phrase-memory-and-choice-bias.md" ] || [ -f "$ROOT_DIR/docs/plans/completed/0135-committed-phrase-memory-and-choice-bias.md" ]; then
    check_cmd "cd '$ROOT_DIR' && rg -n 'KeyboardCommittedPhraseMemory|FretCommittedPhraseMemory|reset\\(self: \\*KeyboardCommittedPhraseMemory\\)|reset\\(self: \\*FretCommittedPhraseMemory\\)|push\\(self: \\*KeyboardCommittedPhraseMemory|push\\(self: \\*FretCommittedPhraseMemory' src/playability/phrase.zig src/tests/playability_phrase_test.zig >/dev/null" "0135 committed phrase memory foundation guardrail (caller-owned committed phrase windows and focused tests are wired)"
    check_cmd "cd '$ROOT_DIR' && rg -n 'rankKeyboardNextStepsFromCommittedPhrase|rankKeyboardContextSuggestionsFromCommittedPhrase|suggestSaferKeyboardNextStepFromCommittedPhrase|preview does not mutate committed memory|accepted choices bias later ranking|committed phrase memory' src/playability/ranking.zig src/playability/profile.zig src/tests/playability_ranking_test.zig src/tests/playability_phrase_test.zig >/dev/null" "0135 committed phrase memory algorithm guardrail (committed-memory bias helpers and semantic tests are implemented)"
    check_cmd "cd '$ROOT_DIR' && rg -n 'lmt_keyboard_committed_phrase_memory|lmt_fret_committed_phrase_memory|lmt_keyboard_committed_phrase_reset|lmt_keyboard_committed_phrase_push|lmt_keyboard_committed_phrase_len|lmt_fret_committed_phrase_reset|lmt_fret_committed_phrase_push|lmt_fret_committed_phrase_len|lmt_audit_committed_keyboard_phrase_n|lmt_audit_committed_fret_phrase_n|lmt_rank_keyboard_next_steps_by_committed_phrase|lmt_rank_keyboard_context_suggestions_by_committed_phrase|lmt_suggest_safer_keyboard_next_step_by_committed_phrase' include/libmusictheory.h src/c_api.zig build.zig scripts/check_wasm_exports.mjs src/tests/c_api_test.zig >/dev/null" "0135 committed phrase memory ABI guardrail (memory structs, push/reset helpers, and committed-bias exports are wired)"
...
if [ -f "$ROOT_DIR/docs/plans/in_progress/0140-phrase-branch-state-and-fixed-horizon-candidate-windows.md" ] || [ -f "$ROOT_DIR/docs/plans/completed/0140-phrase-branch-state-and-fixed-horizon-candidate-windows.md" ]; then
    check_cmd "cd '$ROOT_DIR' && rg -n 'MAX_PHRASE_BRANCH_STEPS|MAX_BRANCH_STEP_CANDIDATES|KeyboardPhraseBranch|FretPhraseBranch|KeyboardPhraseStepCandidates|FretPhraseStepCandidates|KeyboardPhraseCandidateWindow|FretPhraseCandidateWindow|PhraseBranchSummary|summarizeBranchIssues|summarizeKeyboardBranch|summarizeFretBranch|branch summary tracks blocked step peak strain and window trends|keyboard branch summary helper evaluates fixed branch windows' src/playability/phrase.zig >/dev/null" "0140 phrase branch foundation guardrail (fixed-horizon branch structs, candidate windows, summaries, and focused tests are wired)"
    check_cmd "cd '$ROOT_DIR' && rg -n 'lmt_keyboard_phrase_branch|lmt_fret_phrase_branch|lmt_keyboard_phrase_step_candidates|lmt_fret_phrase_step_candidates|lmt_keyboard_phrase_candidate_window|lmt_fret_phrase_candidate_window|lmt_playability_phrase_branch_summary|lmt_sizeof_keyboard_phrase_branch|lmt_sizeof_fret_phrase_branch|lmt_sizeof_keyboard_phrase_step_candidates|lmt_sizeof_fret_phrase_step_candidates|lmt_sizeof_keyboard_phrase_candidate_window|lmt_sizeof_fret_phrase_candidate_window|lmt_sizeof_playability_phrase_branch_summary|lmt_summarize_playability_phrase_branch_issues|lmt_summarize_keyboard_phrase_branch_n|lmt_summarize_fret_phrase_branch_n' include/libmusictheory.h src/c_api.zig build.zig scripts/check_wasm_exports.mjs src/tests/c_api_test.zig >/dev/null" "0140 phrase branch ABI guardrail (branch/window structs, reflection helpers, and summary exports are wired)"
    check_cmd "cd '$ROOT_DIR' && rg -n 'PhraseBranchSummary|fixed-horizon branch|candidate windows do not rank or choose anything|no branch-ranking policy yet|lmt_summarize_playability_phrase_branch_issues|lmt_summarize_keyboard_phrase_branch_n|lmt_summarize_fret_phrase_branch_n' docs/research/algorithms/playability.md docs/api.md >/dev/null" "0140 phrase branch docs guardrail (research notes and unified API docs explain the structural branch boundary)"
fi

if [ -f "$ROOT_DIR/docs/plans/in_progress/0141-phrase-biased-branch-ranking-and-hard-filter-helpers.md" ] || [ -f "$ROOT_DIR/docs/plans/completed/0141-phrase-biased-branch-ranking-and-hard-filter-helpers.md" ]; then
    check_cmd "cd '$ROOT_DIR' && rg -n 'PhraseBranchClassification|PhraseBranchVisibility|PhraseBranchBiasReason|PhraseBranchBiasSummary|RankedKeyboardPhraseBranch|RankedFretPhraseBranch|classifyBranchSummary|rankKeyboardPhraseBranchesFromCommittedPhrase|rankFretPhraseBranchesFromCommittedPhrase|filterBlockedKeyboardPhraseBranchesFromCommittedPhrase|filterBlockedFretPhraseBranchesFromCommittedPhrase|seeded keyboard branch summary reflects committed shift pressure|diagnostic reranking keeps blocked branches visible while hard filter removes them' src/playability/ranking.zig src/playability/phrase.zig src/tests/playability_ranking_test.zig >/dev/null" "0141 phrase branch ranking foundation guardrail (classification, bias summaries, ranking helpers, filter helpers, and focused tests are wired)"
    check_cmd "cd '$ROOT_DIR' && rg -n 'lmt_playability_phrase_branch_class|lmt_playability_phrase_branch_visibility|lmt_playability_phrase_branch_bias_reason|lmt_playability_phrase_branch_bias_summary|lmt_ranked_keyboard_phrase_branch|lmt_ranked_fret_phrase_branch|lmt_playability_phrase_branch_class_count|lmt_playability_phrase_branch_class_name|lmt_playability_phrase_branch_visibility_count|lmt_playability_phrase_branch_visibility_name|lmt_playability_phrase_branch_bias_reason_count|lmt_playability_phrase_branch_bias_reason_name|lmt_sizeof_playability_phrase_branch_bias_summary|lmt_sizeof_ranked_keyboard_phrase_branch|lmt_sizeof_ranked_fret_phrase_branch|lmt_rank_keyboard_phrase_branches_by_committed_phrase|lmt_rank_fret_phrase_branches_by_committed_phrase|lmt_hard_filter_keyboard_phrase_branches_by_committed_phrase|lmt_hard_filter_fret_phrase_branches_by_committed_phrase' include/libmusictheory.h src/c_api.zig build.zig scripts/check_wasm_exports.mjs src/tests/c_api_test.zig >/dev/null" "0141 phrase branch ranking ABI guardrail (branch-ranking enums, structs, size helpers, exports, and C tests are wired)"
    check_cmd "cd '$ROOT_DIR' && rg -n 'hard blockers and soft strain bias separate|blocked only because you asked for hard blockers to be removed|compounds the same shift strain already present in committed memory|lmt_rank_keyboard_phrase_branches_by_committed_phrase|lmt_hard_filter_keyboard_phrase_branches_by_committed_phrase' docs/research/algorithms/playability.md docs/api.md >/dev/null" "0141 phrase branch ranking docs guardrail (research notes and unified API docs explain diagnostics reranking versus hard filtering)"
fi

if [ -f "$ROOT_DIR/docs/plans/in_progress/0142-rewrite-exploration-and-preservation-aware-branch-repairs.md" ] || [ -f "$ROOT_DIR/docs/plans/completed/0142-rewrite-exploration-and-preservation-aware-branch-repairs.md" ]; then
    check_cmd "cd '$ROOT_DIR' && rg -n 'RankedKeyboardPhraseBranchRepair|RankedFretPhraseBranchRepair|rankKeyboardPhraseBranchRepairs|rankFretPhraseBranchRepairs|events_touched|notes_changed|first_relieved_bottleneck_step_index|new_dominant_domain|new_dominant_family_index|repair class homogeneous across branch repair list|keyboard branch repairs stay within one preservation class per call|fret branch repairs report first relieved bottleneck metadata' src/playability/repair.zig src/tests/playability_repair_test.zig >/dev/null" "0142 branch repair foundation guardrail (branch-aware repair structs, metadata, ranking helpers, and focused tests are wired)"
    check_cmd "cd '$ROOT_DIR' && rg -n 'lmt_ranked_keyboard_phrase_branch_repair|lmt_ranked_fret_phrase_branch_repair|lmt_sizeof_ranked_keyboard_phrase_branch_repair|lmt_sizeof_ranked_fret_phrase_branch_repair|lmt_rank_keyboard_phrase_branch_repairs_n|lmt_rank_fret_phrase_branch_repairs_n' include/libmusictheory.h src/c_api.zig build.zig scripts/check_wasm_exports.mjs src/tests/c_api_test.zig >/dev/null" "0142 branch repair ABI guardrail (branch-repair structs, size helpers, exports, and C tests are wired)"
    check_cmd "cd '$ROOT_DIR' && rg -n 'same sounding notes but changes the physical realization across the branch|changes the voicing register, and that is why the bottleneck disappears at step three|do not silently mix preservation classes in one ranked list|lmt_rank_keyboard_phrase_branch_repairs_n|lmt_rank_fret_phrase_branch_repairs_n' docs/research/algorithms/playability.md docs/api.md >/dev/null" "0142 branch repair docs guardrail (research notes and unified API docs explain preservation-aware branch rewrites)"
fi
...
if [ -f "$ROOT_DIR/scripts/capture_wasm_gallery_screenshots.mjs" ] && [ -f "$ROOT_DIR/docs/release/gallery-capture.md" ]; then
    ...
    if command -v node >/dev/null 2>&1 && command -v npm >/dev/null 2>&1 && command -v python3 >/dev/null 2>&1; then
        ...
        check_cmd "cd '$ROOT_DIR' && test -f zig-out/wasm-gallery-captures/gallery-overview.png && test -f zig-out/wasm-gallery-captures/gallery-hero.png && test -f zig-out/wasm-gallery-captures/scene-midi.png && test -f zig-out/wasm-gallery-captures/scene-midi-playability-guide.png && test -f zig-out/wasm-gallery-captures/scene-midi-playability-piano.png && test -f zig-out/wasm-gallery-captures/scene-midi-playability-fret.png && test -f zig-out/wasm-gallery-captures/scene-set.png && test -f zig-out/wasm-gallery-captures/scene-key.png && test -f zig-out/wasm-gallery-captures/scene-chord.png && test -f zig-out/wasm-gallery-captures/scene-progression.png && test -f zig-out/wasm-gallery-captures/scene-compare.png && test -f zig-out/wasm-gallery-captures/scene-fret.png && test -f zig-out/wasm-gallery-captures/captures.json" "0081 gallery screenshot capture guardrail (expected capture artifacts generated)"
    else
