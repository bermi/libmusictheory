...
test "majmin scales geometry slot table is complete and stable" {
    ...
    try std.testing.expectEqual(@as(usize, 76), geometry.SCALE_GEOMETRY_PATHS.len);

    var i: usize = 0;
    while (i < geometry.SCALE_GEOMETRY_PATH_COUNT) : (i += 1) {
        const path_d = geometry.pathForSlot(i) orelse unreachable;
        try std.testing.expect(path_d.len > 0);
    }

    try std.testing.expect(geometry.pathForSlot(geometry.SCALE_GEOMETRY_PATH_COUNT) == null);
